...
plot_heatmap <- function(heatmap_matrix, title) {
  ...
  ggplot(heatmap_data, aes(Var1, Var2, fill = Freq)) +
    ...
    labs(x = "Education Level", y = "Age Group", fill = "Count of '-'", title = title) +
    theme_minimal() +
...
```
Here, we provided three heatmaps: "Missing Data Heatmap - Both Sexes", "Missing Data Heatmap - Male", "Missing Data Heatmap - Female." Each heatmap summarize the missing data from all 60 data frame with the following logic: if there is a missing value in one dataframe in a particular entry (Doctora Degree & 18 to 24 years), the heatmap will add 1, and we loop through all 60 data frame to add up the number of missing value for each entry.

In the "Missing Data Heatmap - Both Sexes"
